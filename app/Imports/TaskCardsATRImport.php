...
class TaskCardsATRImport implements ToModel, WithHeadingRow
{
    /**
    * @param array $row
    *
    ...
    */
    public function model(array $row)
    {
        ...
        $aircrafts = explode(';',$row['aircrafts']);
        $airplanes = Aircraft::whereIn('name',['ATR72-200','ATR72-500','ATR72-600'])->get();
        ...
        if ($row['skill']) {
            $skill_id = Type::where('code','LIKE','%'.$row['skill'].'%')->where('of','taskcard-skill')->first()->id;
        ...
        // - Table: thresholds
        if($row['threshold'] <> '-'){
            $threshold_type = Type::ofMaintenanceCycle()->where('code', 'like', '%' . $row['threshold_type'] . '%')->first()->id;
            $taskcard->thresholds()->save(new Threshold([
                'amount' => $row['threshold'],
        ...
        // - Table: repeats
        if($row['repeat'] <> '-'){
            $threshold_type = Type::ofMaintenanceCycle()->where('code', 'like', '%' . $row['repeat_type'] . '%')->first()->id;
