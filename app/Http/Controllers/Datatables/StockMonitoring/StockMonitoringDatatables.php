...
namespace App\Http\Controllers\Datatables\StockMonitoring;
...
class StockMonitoringDatatables extends Controller
{
    /**
     ...
     */
    public function partNumber(Item $item)
    {
        $StockMonitoring = FefoIn::with('item','storage','good_received.purchase_order.purchase_request.purchase_requestable','item.categories')->where('item_id',$item->id)->get();

        foreach($StockMonitoring as $stock){
            if(sizeOf($stock->item->categories) <> null){
                $stock->category .= $stock->item->categories->first()->name;
            }
            $stock->unit = $stock->item->unit->name;
            $stock->min = $stock->item->storages->where('pivot.storage_id',$stock->storage_id)->first()->pivot->min;
            $stock->max = $stock->item->storages->where('pivot.storage_id',$stock->storage_id)->first()->pivot->max;
        }
