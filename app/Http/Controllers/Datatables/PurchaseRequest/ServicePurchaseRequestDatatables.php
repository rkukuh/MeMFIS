...
class ServicePurchaseRequestDatatables extends Controller
{
    /**
     ...
     * @return \Illuminate\Http\Response
     */
    public function pr_service(PurchaseRequest $purchaseRequest)
    {
        $services = PurchaseRequestService::with('service','service.unit', 'service.categories')->where('purchase_request_id',$purchaseRequest->id)->whereHas('service', function ($query) {
            $query->whereHas('categories', function ($query2) {
                $query2->whereIn('code', ['service']);
            });
        })->get();

        foreach($services as $service){
            $service->unit_name .= Unit::find($service->unit_id)->name;

            $stock  = new CheckStock;

        }

        $data = $alldata = json_decode($services);
