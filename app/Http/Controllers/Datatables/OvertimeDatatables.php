...
class OvertimeDatatables extends Controller
{
    public function index(){
        $raw_overtime = Overtime::all();
        $attendance = [];

        $i = 0;
        foreach($raw_overtime as $raw_data){
            $statuses = null;
            $employee_data = $raw_data->employee()->get();
            $employee_statuses = $raw_data->statuses()->get();
            ...
            $late = gmdate('H:i:s',$raw_data->late_in);
            $earlier = gmdate('H:i:s',$raw_data->earlier_out);
            $overtime = gmdate('H:i:s',$raw_data->overtime);
            ...
            $attendance[$i] = [
                ...
                'employee_name' => $name,
                'date' => $raw_data->date,
                'days' => date('l', strtotime($raw_data->date)),
                'in' => $raw_data->in,
                'out' => $raw_data->out,
                'late_in' => $late,
                'earlier_out' => $earlier,
                'overtime' => $overtime,
                'second' => $raw_data->overtime,
