    ...
    $('.summary_datatable').mDatatable({
      data: {
        ...
        source: {
          read: {
            method: 'GET',
            url: '/datatables/quotation/' + quotation + '/additional/job-request',
            map: function (raw) {
          ...
          template: function (t) {
            if(currency.id == 1){
              return (t.pivot.description + '<br>' +
                '- Manhours Price : ' + numberFormat.format(t.pivot.manhour_total) + ' x ' + IDRformatter.format(t.pivot.manhour_rate_amount) + '<br>' +
                '- Facility Price : '
              );
            }else{
              return (t.pivot.description + '<br>' +
        ...
        },
        {
          ...
          template: function (t, e, i) {
            ...
            if(t.pivot.discount_value == null && t.pivot.discount_type == null){
                ...
                if(t.pivot.discount_type ==  'amount'){
                    total = t.total_manhours_with_performance_factor * t.pivot.manhour_rate_amount + t.facilities_price_amount + t.mat_tool_price - t.pivot.discount_value;
                    subtotal = subtotal + total;

                }
                else if(t.pivot.discount_type == 'percentage'){
                    total = t.total_manhours_with_performance_factor * t.pivot.manhour_rate_amount + t.facilities_price_amount + t.mat_tool_price - (((t.total_manhours_with_performance_factor * t.pivot.manhour_rate_amount + t.facilities_price_amount + t.mat_tool_price)*t.pivot.discount_value)/100);
                    subtotal = subtotal + total;
                }
            }
