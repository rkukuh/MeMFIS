                                ...
                                <div class="form-group m-form__group row">
                                    <div class="col-sm-12 col-md-12 col-lg-12">
                                        <table class="table table-bordered second" widtd="100%" cellpadding="4">
                                            ...
                                            @if (in_array($benefit->name, $search))
                                            @php
                                            $key = array_search($benefit->name, $search)
                                            @endphp
                                            <tr>
                                                <td align="center" style="padding-top:25px;">
                                                    @component('frontend.common.input.checkbox')
                                                        @slot('id', str_replace(' ', '', $benefit->uuid))
                                                        @slot('name', 'check')
                                                        @slot('checked','checked')
                                                        @slot('value', $benefit->uuid)
                                                        @slot('onclick','checkboxFunction(this.id)')
                                                        @slot('id_error', str_replace(' ', '', $benefit->uuid))
                                                    @endcomponent
                                                </td>
                                                <td style="padding-top:30px;"><b>{{ $benefit->name }}</b></td>
                                                <td>
                                                    @component('frontend.common.input.number')
                                                        @slot('name', 'position_min')
                                                        @slot('id', str_replace(' ', '', $benefit->uuid).'_min')
                                                        @slot('value', $min[$key])
                                                        @slot('id_error', str_replace(' ', '', $benefit->uuid).'_min')
                                                    @endcomponent
                                                </td>
                                                <td>
                                                    @component('frontend.common.input.number')
                                                        @slot('name', 'position_max')
                                                        @slot('id', str_replace(' ', '', $benefit->uuid).'_max')
                                                        @slot('value', $max[$key])
                                                        @slot('id_error', str_replace(' ', '', $benefit->uuid).'_max')
                                                    @endcomponent
                                                </td>
                                            </tr>
                                            @else
